...
func (uc *userControllerInterface) FindUserByID(c *gin.Context) {
	...
	if _, err := bson.ObjectIDFromHex(userId); err != nil {
		...
		errorMessage := rest_err.NewBadRequestError("UserID is not a valid user id")
	...
	if err != nil {
		logger.Error("Error trying to call service findUserByIDServices", err, zap.String("journey", "findUserByID"))
		c.JSON(err.Code, err)
		return
	}
...
func (uc *userControllerInterface) FindUserByEmail(c *gin.Context) {
	...
	if err != nil {
		logger.Error("Error trying to call service findUserByEmailServices", err, zap.String("journey", "findUserByEmail"))
		c.JSON(err.Code, err)
		return
	}
